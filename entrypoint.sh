...
install_pip_package () {
  ...
  gosu sopel pip install --user "${PACKAGE}" || {
    RC=$?
    echo -e "\033[41mFAILED!\033[0m" && return $RC
  }
}
...
install_apt_packages () {
  PACKAGES="${@}"
  echo -e "\033[44mInstalling apt packages: ${PACKAGES}...\033[0m"
...
# Run sopel
if [ "${1}" = "sopel" ]; then
  # Install extra apt packages first to satisfy pip requirements
  [ -f "/apt_packages.txt" ] && install_apt_packages $(cat /apt_packages.txt | grep -v '^#' | grep -v '^$' | tr $'\n' ' ')

  [ -n "${EXTRA_APT_PACKAGES}" ] && install_apt_packages ${EXTRA_APT_PACKAGES}
